...
softening = 1E-8       # Scales down forces for very close objects - to avoid approaching infinity (1E-8 is ok for this system). 
...
sun       = kutta2(softening=softening,body_list=Body_list,light=True,c=color.yellow,draw=False) # draw=False boosts the calculation, use this in every object to disable all animations
earth     = kutta2(x=1,r=10,softening=softening,texture=textures.earth,mass=3.003e-06,label="Earth",body_list=Body_list,draw=False)  # 1AU -distance from earth to sun, softening prevents rapid changes in acceleration (earth-moon system)     
jupiter   = kutta2(x=5.2,r=17,c=color.orange,mass=9.551e-04,label="Jupiter",body_list=Body_list,draw=False)
moon      = kutta2(x=1.00256954,r=10,c=color.white,mass=3.69369e-08,label="Moon",body_list=Body_list,draw=False)
venus     = kutta2(x=0.723,r=10,c=color.red,mass=3.01035735e-08,label="Venus",body_list=Body_list,draw=False)
asteroid  = kutta2(x=1.5,r=7,vz=0.005,c=color.green,mass=1e-08,label="asteroid",body_list=Body_list,draw=False) #velocities of these bodies are explicitly stated here.
asteroid2 = kutta2(y=2,r=10,vx=0.005,vz=0.005,c=color.purple,mass=8.4e-08,label="asteroid 2",body_list=Body_list,draw=False)
